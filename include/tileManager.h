...
#define xt_iss_switch_mode
#define xt_profile_enable()
#define xt_profile_disable()
#undef XT_RSR_CCOUNT
...
typedef int32_t  idma_max_pif_t;
typedef int32_t  idma_ticks_cyc_t;
typedef int32_t  idma_error_details_t;
typedef int32_t  idma_buffer_t;
typedef void (*idma_callback_fn)(void*);
typedef void (*idma_err_callback_fn)(idma_error_details_t*);
typedef int32_t  xmem_mgr_t;
typedef int32_t  xmem_status_t;
typedef int32_t  _idma_buffer_t;
...
#define SETUP_FRAME(pFrame, pFrameBuffer, buffSize, width, height, pitch, padWidth, padHeight, pixRes, numCh, paddingType, paddingVal) \
  {                                                                                                                                    \
    ...
    XV_FRAME_SET_DATA_PTR((xvFrame *) (pFrame), (pFrameBuffer) + ((pitch) * (padHeight) + (padWidth) * (numCh)) * (pixRes));           \
