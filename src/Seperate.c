...
}Seperate_UV_Context, *pSeperate_UV_Context;

/*
 * Initialize context.
 */
//Seperate_UV_InitializeContext(&ctx , in1 , in2 , alpha , out , width , height , pitch);
static ErrorType Seperate_UV_InitializeContext(Seperate_UV_Context *ctx , uint8_t *pUV , uint8_t *pU , uint8_t *pV ,
 ...
 */
static inline ErrorType ScheduleInputDMA(Seperate_UV_Context *ctx, xi_point *srcPos ,  xi_point *dstPos , int phase)
{
	xvTileManager *pTM = &ctx->tileMgr;
...
void Seperate_UV_Cadence(Seperate_UV_Context *ctx , int phase)
{
...
void Seperate_UV_C(Seperate_UV_Context *ctx , int phase)
{
	xvTile *pTileUV = ctx->tileIn[phase];
	xvTile *pTileU  = ctx->tileOut[phase][0];
	xvTile *pTileV  = ctx->tileOut[phase][1];
...
static ErrorType Seperate_UV_Process(Seperate_UV_Context *ctx)
{
...
#pragma frequency_hint never
	...
	for (i = 0; i < numTiles; i++) {
		...
		Seperate_UV_Cadence(ctx , bufIndex);
#else
		Seperate_UV_C(ctx , bufIndex);
#endif
 ...
 * Release context.
 */
static ErrorType Seperate_UV_ReleaseContext(Seperate_UV_Context *ctx)
{
	xvTileManager *pTM = &ctx->tileMgr;
...
{
	Seperate_UV_Context ctx;

	Seperate_UV_InitializeContext( &ctx , pUV , pU , pV , width , height , stride);
