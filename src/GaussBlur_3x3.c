...
//定义放缩,1/9 * 2^15(32768) = 3640.8888，除数大一些，防止数据越界超过255
#define Q15_1div9	3641

//GaussBlur的半径
#define GAUSS_BLUR_3x3_RADIUS		1
...
}GaussBlur_3x3_Context, *pGaussBlur_3x3_Context;

/*
 * Initialize context.
 */
static ErrorType GaussBlur3x3_InitializeContext(GaussBlur_3x3_Context *ctx , void *pSrc , int swidth , int sheight , int spitch ,
		void *pDst , int dwidth , int dheight , int dpitch)
{
	...
	ctx->srcTile.pitch  = GAUSS_BLUR_TILE_PITCH + 2 * GAUSS_BLUR_3x3_RADIUS;
	...
	int tileInputBuffSize  = (ctx->srcTile.pitch + 2*GAUSS_BLUR_3x3_RADIUS) * (ctx->srcTile.height + 2*GAUSS_BLUR_3x3_RADIUS) + 64;
	...
	SETUP_TILE(ctx->tileBuffer[0].pIn , ctx->pdramIn[0], tileInputBuffSize, ctx->pSrcFrame, ctx->srcTile.width,
			ctx->srcTile.height, ctx->srcTile.pitch, XV_TILE_U8, GAUSS_BLUR_3x3_RADIUS, GAUSS_BLUR_3x3_RADIUS, 0, 0, DATA_ALIGNED_64);
 ...
 */
static inline ErrorType ScheduleInputDMA(GaussBlur_3x3_Context *ctx, xi_point *srcPos ,  xi_point *dstPos, TileBuffer *tileBuf)
{
	xvTileManager *pTM = &ctx->tileMgr;
...
//Y-component GaussBlur , size = 3x3,cadence版本
static ErrorType GaussBlur_Y_3x3_U8_Cadence(TileBuffer *buffer)
{
	...
	xb_vec2Nx8U *psrc = OFFSET_PTR_2NX8U( XI_TILE_GET_DATA_PTR(pTileIn) , -GAUSS_BLUR_3x3_RADIUS , sstride , -GAUSS_BLUR_3x3_RADIUS );
	...
	for( ; j < width - XCHAL_IVPN_SIMD_WIDTH ; j += 2 * XCHAL_IVPN_SIMD_WIDTH , psrc += 1 , pdst += 1) {
		...
		xb_vec2Nx8U v00 , v01 , v10 , v11 , v20 , v21;

		int offs = sstride - MIN(4 * XCHAL_IVPN_SIMD_WIDTH , width + 2 - j);
		...
		{   // row 0
			...
			IVP_LAV2NX8U_XP(v00, a_load, rsrc, width + 2 - j);
			IVP_LAV2NX8U_XP(v01, a_load, rsrc, width + 2 - j - 2 * XCHAL_IVPN_SIMD_WIDTH);
			rsrc = OFFSET_PTR_2NX8U(rsrc , 1 , offs , 0);
		}

		{   // row 1
			valign a_load = IVP_LA2NX8U_PP(rsrc);
			IVP_LAV2NX8U_XP(v10, a_load, rsrc, width + 2 - j);
			IVP_LAV2NX8U_XP(v11, a_load, rsrc, width + 2 - j - 2 * XCHAL_IVPN_SIMD_WIDTH);
			rsrc = OFFSET_PTR_2NX8U(rsrc , 1 , offs , 0);
		}

		for(i = 0 ; i < height ; i++){
			valign a_load = IVP_LA2NX8U_PP(rsrc);
			xb_vec2Nx8U v20; IVP_LAV2NX8U_XP(v20, a_load, rsrc, width + 2 - j);
			xb_vec2Nx8U v21; IVP_LAV2NX8U_XP(v21, a_load, rsrc, width + 2 - j - 2 * XCHAL_IVPN_SIMD_WIDTH);
			rsrc = OFFSET_PTR_2NX8U(rsrc , 1 , offs , 0);

			//一次计算三次的乘加/
			xb_vec2Nx24 w = IVP_MULUS4T2N8XR8(v01, v00, 0x00010201);
			IVP_MULUS4TA2N8XR8(w, v11, v10, 0x00020402);
			IVP_MULUS4TA2N8XR8(w, v21, v20, 0x00010201);
			xb_vec2Nx8U vdst = IVP_PACKVRU2NX24(w , 4);

			IVP_SAV2NX8U_XP(vdst, a_st, rdst, width - j);
			IVP_SAPOS2NX8U_FP(a_st , rdst);
			rdst = OFFSET_PTR_2NX8U(rdst, 1, offd, 0);

			//reuse the register(avoid to re-load)
			v00 = v10;	v01 = v11;
			v10 = v20;	v11 = v21;
		}
	}
	//确保最后一个output的宽度小于SIMD_WIDTH(32)
	if(j < width){

		rsrc = psrc;
		rdst = pdst;
//		xb_vecNx8U* __restrict rdst_2 = (xb_vecNx8U *)pdst;

		int offs = sstride - (width + 2 - j);
		int offd = dstride - (width - j);

		xb_vec2Nx8U v00 , v10 , v20;
		{ 	// row 0
			valign a_load = IVP_LA2NX8U_PP(rsrc);
			IVP_LAV2NX8U_XP(v00 , a_load , rsrc , width + 2 - j);
			rsrc = OFFSET_PTR_2NX8U(rsrc , 1 , offs , 0);
		...
		for(i = 0 ; i < height ; i++){
			...
			valign a_load = IVP_LA2NX8U_PP(rsrc);
			IVP_LAV2NX8U_XP(v20 , a_load , rsrc , width + 2 - j);
			rsrc = OFFSET_PTR_2NX8U(rsrc , 1 , offs , 0);

			//一次计算三次的乘加
			xb_vec2Nx24 w = IVP_MULUS4T2N8XR8(0, v00, 0x00010201);
			IVP_MULUS4TA2N8XR8(w, 0, v10, 0x00020402);
			IVP_MULUS4TA2N8XR8(w, 0, v20, 0x00010201);
			xb_vec2Nx8U vdst = IVP_PACKVRU2NX24(w , 4);
			//vdst = 0;
...
//Y-component GaussBlur , size = 3x3,C版本
static ErrorType GaussBlur_Y_3x3_U8_C(TileBuffer *buffer)
{
	xvTile *pTileIn  = buffer->pIn;
	xvTile *pTileOut = buffer->pOut;
...
//	uint8_t *psrc = (uint8_t *)(XI_TILE_GET_DATA_PTR(pTileIn) - 1 * sstride  - GAUSS_BLUR_3x3_RADIUS);
	...
	for(j = 0 ; j < height ; j++){
		for(i = 0 ; i < width ; i++){
			...
			int offset = j * dstride + i;

			arr[0] = psrc[(j - 1) * sstride + i - 1];
			arr[1] = psrc[(j - 1) * sstride + i];
			arr[2] = psrc[(j - 1) * sstride + i + 1];
			arr[3] = psrc[j * sstride + i - 1];
			arr[4] = psrc[j * sstride + i];
			arr[5] = psrc[j * sstride + i + 1];
			arr[6] = psrc[(j + 1) * sstride + i - 1];
			arr[7] = psrc[(j + 1) * sstride + i];
			arr[8] = psrc[(j + 1) * sstride + i + 1];
 ...
 * Process context.
 */
static ErrorType GaussBlur_3x3_Process(GaussBlur_3x3_Context *ctx)
{
...
#ifdef CADENCE_OPT_GAUSS_BLUR_3x3
		GaussBlur_Y_3x3_U8_Cadence(pTileBuffer);
#else
		GaussBlur_Y_3x3_U8_C(pTileBuffer);
 ...
 * Release context.
 */
static ErrorType GaussBlur_3x3_ReleaseContext(GaussBlur_3x3_Context *ctx)
{
	xvTileManager *pTM = &ctx->tileMgr;
...
void GaussBlur_3x3_U8(uint8_t *src , uint8_t *dst , int width , int height , int pitch)
{

	GaussBlur_3x3_Context ctx;

	GaussBlur3x3_InitializeContext(&ctx , src , width , height , pitch ,dst , width , height , pitch);


	GaussBlur_3x3_Process(&ctx);

	GaussBlur_3x3_ReleaseContext(&ctx);
